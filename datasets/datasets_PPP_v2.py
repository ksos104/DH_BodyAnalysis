...
class PascalPartSegmentation(data.Dataset):
    def __init__(self, root, split, crop_size=[473, 473], scale_factor=0.25,
        ...
        # self.file_names = os.listdir(os.path.join(self.root,'val','image'))
    ...
    def __getitem__(self, index):
        ...
        # im_path = os.path.join(self.root, 'val' , 'image', item)
        # parsing_anno_path = os.path.join(self.root, 'val' , 'segmentation', item)
        ...
        if self.dataset == 'val' or self.dataset == 'sample':
...
if __name__ == '__main__':
    ...
    train_dst = PascalPartSegmentation(root='../../data/PPP', split='train', crop_size=[512, 512], scale_factor=0.25,
    ...
    val_dst = PascalPartSegmentation(root='../../data/PPP', split='val', crop_size=[512, 512], scale_factor=0.25,
                                       rotation_factor=30, ignore_label=255, flip_prob=0.5, transform=transform,
                                       void_pixels=3, return_edge=return_edge)

    val_loader = data.DataLoader(train_dst, batch_size=32, shuffle=False, num_workers=2)
    ...
    for (images, labels) in tqdm(train_loader):
        ...
        pdb.set_trace() 
        plt.imshow(images[16].permute(1,2,0))
        plt.show()
        pdb.set_trace()
