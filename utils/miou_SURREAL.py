...
def compute_mean_ioU(preds, scales, centers, num_classes, datadir, input_size=[473, 473], dataset='test'):

    # datadir = '../data/pascal_data_15000'

    file_names = os.listdir(os.path.join(datadir,dataset,'segmentation'))
    # item = self.file_names[index]


    # im_path = os.path.join(datadir, dataset , 'image', item)
    # parsing_anno_path = os.path.join(datadir, dataset , 'segmentation', item)

    # val_file = os.path.join(datadir, 'val',dataset + '_id.txt')
    ...
    for i, pred_out in enumerate(preds):
        ...
        # im_name = val_id[i]
        gt_path = os.path.join(datadir, dataset, 'segmentation', im_name)
